...
/* get canvas dimensions and fill the whole window */
...
/*** move particle utilities ***/
Particle.prototype.move = function () {
	//add current accel to current velocity
	this.velocity.add(this.acceleration);
	//add current velocity to our position
...
function addNewParticles() {
	...
	for (var i=0; i < emitters.length; i++) {
		...
		//for emissionRate, emit particle
...
/*** end particle utilities ***/

/*** Plot particles function ***/
function plotParticles(boundsX, boundsY) {
	//new array to hold bounded particles
	...
	for(var i=0; i<particles.length; i++) {
		...
		//if out of bounds --> drop particle, move to next
		if (pos.x < 0 || pos.x > boundsX || pos.y < 0 || pos.y > boundsY) continue;

		//update velocities and acc to acct for fields
...
function drawParticles() {
	...
	//for each particle
	for (var i=0; i < particles.length; i++) {
		var position = particlesp[i].position;

		//draw square @ particleSize posiion
